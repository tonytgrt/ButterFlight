...
% Figures: design-doc figures live in ../Design_Doc/img/, new
% Final-Report screenshots live in ./img/.
\graphicspath{{./}{../Design_Doc/}}

% ---- Section formatting (mirrors the Design Doc) ----
\titleformat{\section}{\large\bfseries}{{\thesection.}}{0.5em}{}
\titleformat{\subsection}{\normalsize\bfseries}{{\thesubsection}}{0.5em}{}
\titleformat{\subsubsection}{\normalsize\bfseries}{{\thesubsubsection}}{0.5em}{}

\title{ButterFlight: Butterfly Flight Authoring Tool \\ \large Final Report}
\author{Cecilia Chen \and Yiding Tian}
\date{}
...
% ============================================================
%   This Final Report extends the original Design Doc with the
%   Results, Discussion, and Lessons Learned sections required by
%   the rubric.  Sections 1--4 below are migrated verbatim from
%   ButterFlight_DesignDoc.tex; Sections 5--7 are new.
% ============================================================
...
\begin{itemize}
  ...
  \item \textbf{Parameter tuning.} Adjust physical parameters
...
containing seven collapsible \texttt{frameLayout} sections---one per logical parameter
...
\begin{figure}[H]
  ...
  \caption{ButterFlight v0.1 UI panel running inside Maya 2026. Sections 3 (Path
    Settings) and 6 (Swarm Settings) are hidden by default and become visible
...
The panel is divided into seven numbered, collapsible sections plus an action-button
...
3 & Path Settings       & Curve picker + follow-strength slider {\small(conditional)} \\
4 & Wind                & Enable check; direction XYZ; magnitude slider; time-varying \\
5 & Force Parameters    & Vortex gain X/Y/Z; $\eta$; max speed; smoothing window $k$ \\
6 & Swarm Settings      & Agent count; spawn spread; repulsion radius {\small(conditional)} \\
...
\textbf{Section 2 --- Simulation Mode.}
An \texttt{optionMenuGrp} controls the overall simulation behavior. Switching modes
triggers the \texttt{bfUpdateMode} callback, which shows or hides Sections~3 and~6:
Section~3 (Path Settings) is visible only in \textit{Path Following} mode; Section~6
(Swarm Settings) is visible only in \textit{Swarm} mode.

\textbf{Section 3 --- Path Settings (conditional).}
...
\textbf{Section 5 --- Force Parameters.}
...
scale $\eta = 3.66$ are pre-set to the defaults from the paper and are editable
for fine-tuning. Maximum flight speed (m/s) and sliding-window size $k$ (default 10
frames) complete this section.

\textbf{Section 6 --- Swarm Settings (conditional).}
...
\textbf{Section 7 --- Output Settings.}
...
\begin{itemize}
  ...
  \item \textbf{Bake to Keys} (blue): Calls \texttt{bakeResults} on the entire rig
    ...
    also triggered.
  \item \textbf{Reset} (red): Restores all controls to their default values and clears
    the rig and path fields.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Select Simulation Mode.} The Section~2 drop-down switches between
  ...
  \item \textbf{Set Path Curve (Path Following mode only).} The user selects an existing
    NURBS curve in the viewport and clicks \textit{Select} in Section~3 to designate it
  ...
  \item \textbf{Enable and Configure Wind.} Checking the \textit{Enable Wind} box in
    Section~4 activates direction and magnitude controls. The user sets a world-space
    direction vector and a magnitude (m/s), and can optionally enable time-varying
    sinusoidal wind to simulate gusts.

  \item \textbf{Tune Force Parameters.} Section~5 exposes the six low-level numerical
  ...
  \item \textbf{Configure Swarm (Swarm mode only).} Section~6 lets the user specify the
    number of agents, their spatial spread radius at spawn, and the inter-agent repulsion
    radius to prevent overlap.

  \item \textbf{Set Output Settings.} Section~7 defines the simulation time range (start
  ...
  \item \textbf{Reset.} The red \textit{Reset} button clears all input fields and
    restores every control to its factory default, including hiding the conditional
    sections.
\end{itemize}
...
Section~5 carry sensible defaults and need not be touched for typical use.
...
\node[proc, below=of assign]        (params)  {Set simulation mode and parameters};
...
\caption{ButterFlight workflow. After opening the panel and assigning a rig, the user
  configures mode and simulation parameters, runs the simulation, then
...
\begin{itemize}
  \item \textbf{\texttt{BFSimulateCmd} (subclass of \texttt{MPxCommand})}.
    Parses all simulation flags (rig root, mode, physical parameters, force
...
\begin{itemize}
  ...
  \item \textbf{BFWingModel} --- Evaluates Equations~1--3 each frame to compute
    ...
    physical parameters: mass, wing area, nominal flapping frequency,
  ...
  \item \textbf{BFCurlNoise} --- Generates the curl of a 3-D Perlin noise field
    ...
    Applies the gain and $\eta$ constants to produce the vortex
...
\begin{itemize}
  ...
  \item \textbf{Fixed physical parameters} --- mass $0.428\,$g,
    wing area $26\,$cm$^2$, and nominal flapping parameters derived from
    Monarch butterfly data in Chen et al.
...
export integration. It targets the milestone date of \textbf{April 15} and
...
\begin{itemize}
  ...
  \item \textbf{Full MEL UI panel} --- the Wind and Swarm Settings
...
\begin{itemize}
  \item \textbf{Subtask 1.1 --- CMake Build Configuration} \textit{(2 days --- Cecilia Chen, Yiding Tian)}: Configure
  ...
  \item \textbf{Subtask 1.2 --- BFSimulateCmd Skeleton} \textit{(2 days --- Cecilia Chen, Yiding Tian)}: Implement and register a
  ...
  \item \textbf{Subtask 1.3 --- MEL Round-Trip Verification} \textit{(1 day --- Cecilia Chen, Yiding Tian)}: Write the opening
...
\begin{itemize}
  \item \textbf{Subtask 2.1 --- BFState Struct and Joint Enum} \textit{(2 days --- Yiding Tian)}: Define the
  ...
  \item \textbf{Subtask 2.2 --- Maneuvering Function (Eqs.~1--3)} \textit{(3 days --- Yiding Tian)}: Implement
    ...
    amplitude modifiers, seeded with the nominal values from
    Table~\ref{tab:params}.
  \item \textbf{Subtask 2.3 --- Joint-Rotation Applicator} \textit{(2 days --- Yiding Tian)}: Write the helper that
  ...
  \item \textbf{Subtask 2.4 --- Visual Verification} \textit{(3 days --- Yiding Tian)}: Manually scrub a 60-frame
...
\begin{itemize}
  \item \textbf{Subtask 3.1 --- Wing-Panel Normal Computation} \textit{(2 days --- Cecilia Chen)}: Approximate each
  ...
  \item \textbf{Subtask 3.2 --- Lift/Drag Polynomial Evaluation (Eqs.~4--6)} \textit{(3 days --- Cecilia Chen)}:
  ...
  \item \textbf{Subtask 3.3 --- Newton Integration} \textit{(3 days --- Cecilia Chen)}: Accumulate net aerodynamic
...
\begin{itemize}
  \item \textbf{Subtask 4.1 --- 3-D Gradient Perlin Noise} \textit{(2 days --- Cecilia Chen)}: Implement a
  ...
  \item \textbf{Subtask 4.2 --- Finite-Difference Curl (Eq.~7)} \textit{(2 days --- Cecilia Chen)}: Compute
  ...
  \item \textbf{Subtask 4.3 --- UI Integration} \textit{(1 day --- Cecilia Chen)}: Wire the Wind section of the MEL
...
\begin{itemize}
  \item \textbf{Subtask 5.1 --- Preferred Acceleration (Eq.~8)} \textit{(3 days --- Yiding Tian)}: Implement the
  ...
  \item \textbf{Subtask 5.3 --- Sliding-Window Smoother (Eq.~12)} \textit{(3 days --- Yiding Tian)}: Implement the
...
frameLayout sections, mode-dependent section visibility,
...
\begin{itemize}
  \item \textbf{Subtask 6.1 --- Full Control Layout} \textit{(2 days --- Cecilia Chen)}: Implement all seven
    frameLayout sections (Rig Assignment, Simulation Mode, Path
  ...
  \item \textbf{Subtask 6.2 --- Callbacks} \textit{(1 day --- Cecilia Chen)}:
    Implement \texttt{bfUpdateMode}
    (show/hide Path and Swarm sections) and \texttt{bfWindToggle}
    (enable/disable gain sliders).
  \item \textbf{Subtask 6.3 --- Command Invocation and Bake}\\
    \textit{(2 days --- Cecilia Chen)}: Implement
...
\begin{itemize}
  \item \textbf{Subtask 7.1 --- End-to-End Integration Test} \textit{(1 day --- Cecilia Chen, Yiding Tian)}: Load the plugin and
  ...
  \item \textbf{Subtask 7.2 --- Integration Bug Fixes} \textit{(1 day --- Cecilia Chen, Yiding Tian)}: Resolve any discrepancies
  ...
  \item \textbf{Subtask 7.3 --- Alpha Scene Packaging} \textit{(1 day --- Cecilia Chen, Yiding Tian)}: Save the verified scene as
...
and export integration. All alpha tasks remain complete; the following
...
per-agent wing-motion variation; and (3)~click \textit{Export Cache} and receive a
valid \texttt{.abc} or \texttt{.fbx} file.

\subsection{Schedule}

Figure~\ref{fig:gantt} shows the full project schedule. Each coloured bar indicates
the assigned team member(s): \textbf{C}~=~Cecilia Chen, \textbf{Y}~=~Yiding Tian,
\textbf{C\&Y}~=~both. Milestone markers: $*1$ Alpha (Mar~25),
$*2$ Beta (Apr~15), $*3$ Final Demo / Code Due (May~5 / May~11).

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{img/Gantt.png}
  \caption{ButterFlight project schedule (Weeks 1--11, Feb~25 -- May~11).}
  \label{fig:gantt}
\end{figure}
...
physical forces for realistic simulation. Later works extended these ideas with
biologically-motivated spatial zones (repulsion, alignment, attraction), which in
turn led to the insect swarm models that feed into [CLTL22].
...
model for insect swarms. This work, from the same research group (co-authors Jin
and Manocha) as [CLTL22], established the framework for biologically-grounded
insect swarm animation. However, BSwarm and similar swarm methods focus on
macro-level swarm trajectories, using pre-created cycle-frames for individual
insect motion---a limitation that [CLTL22] addresses with its micro-level
force-based model.
...
\bigskip\noindent
\textbf{[CLTJ19]} Chen et al.\ extend the chaotic behavior of flying insects to
generate special-effect animations with shape constraints. Building on the swarm
simulation foundation of [WRJM15] and [WJDZ14], this work demonstrates
artistically controllable insect swarm motion. It serves as a direct predecessor
to [CLTL22], which shifts focus from macro-level swarm effects to a
physically-grounded, micro-level individual butterfly flight model with wing-body
interaction.
...
% ============================================================
%       NEW SECTIONS REQUIRED BY THE FINAL REPORT RUBRIC
% ============================================================
...
% ------------------------------------------------------------
\subsection{Documentation / Tutorial}
% ------------------------------------------------------------

This section walks through the complete artist-facing workflow of ButterFlight,
from loading a rigged butterfly mesh to baking and rendering the final
animation.  The tool is structured around a single MEL panel in Maya and three
exclusive simulation modes (Hover, Path Following, Free Flight), all with
optional swarm replication and cinematic camera support layered on top.

\subsubsection{Step 1: Load the Rigged Butterfly}

The user begins by loading a rigged butterfly mesh into the Maya scene.  The
plug-in expects the standard ButterFlight skeleton structure: a \texttt{BF\_body}
root joint with \texttt{BF\_thorax}, \texttt{BF\_forewing\_L/R},
\texttt{BF\_hindwing\_L/R}, and \texttt{BF\_abdomen} as descendants, plus
optional \texttt{BF\_head}, \texttt{BF\_antenna\_L/R}, and leg joints.  Skin
weights should be bound to each wing/body part with the ``Bind to Selected
joints'' option to avoid cross-influence.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/skeleton_structure.png}
%   \fbox{\parbox{0.65\textwidth}{\centering\vspace{3em}
%     \textit{[PLACEHOLDER: \texttt{img/skeleton\_structure.png} ---
%     Maya outliner showing the expected ButterFlight joint hierarchy
%     (\texttt{BF\_body} as root, with \texttt{BF\_thorax}, the four wings,
%     abdomen, head, antennae, and legs).]}\vspace{3em}}}
  \caption{The ButterFlight rigging convention.  Joint names must follow the
    \texttt{BF\_} prefix scheme so the plug-in can locate each joint
    automatically.}
  \label{fig:rig_structure}
\end{figure}

\subsubsection{Step 2: Open the ButterFlight Panel and Assign the Rig}

After loading the plug-in (\texttt{loadPlugin "butterFlight.mll"}) and sourcing
\texttt{butterFlight\_ui.mel}, calling \texttt{butterFlightUI} brings up the main
panel.  The user selects the root joint of the rig in the viewport, then clicks
\textbf{Select} in Section~1 to assign it.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/panel_overview.png}
%   \fbox{\parbox{0.55\textwidth}{\centering\vspace{3em}
%     \textit{[PLACEHOLDER: \texttt{img/panel\_overview.png} ---
%     full ButterFlight panel as it appears on first launch with the rig
%     assigned and Hover mode selected.]}\vspace{3em}}}
  \caption{ButterFlight panel after launch, with the rig root assigned in
    Section~1.}
  \label{fig:panel_overview}
\end{figure}

\subsubsection{Step 3a: Hover Mode}

In Hover mode, ButterFlight keeps the butterfly fixed in space while wing
flapping, abdomen counter-phase oscillation, and an optional positional drift
play out around the chosen anchor.  The user may either reuse the rig's
current world position (default) or enter explicit X/Y/Z coordinates and
roll/pitch/yaw angles.  The \textbf{Enable Hover Noise} option drives a slow
three-axis drift (sum of two slow incommensurate sines per axis,
$\approx 0.2$--$0.5$\,Hz) so the butterfly does not appear locked in place.

In the Output Settings panel, the user sets the \textbf{Start Frame} and
\textbf{Duration} of the animation along with the \textbf{Flap Period} (in
seconds), which controls how rapidly the wings flap.  Clicking \textbf{Simulate}
bakes the animation onto the rig joints over the requested frame range.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/hover_panel.png}
%   \fbox{\parbox{0.50\textwidth}{\centering\vspace{3em}
%     \textit{[PLACEHOLDER: \texttt{img/hover\_panel.png} ---
%     the Hover Settings sub-panel expanded, showing the
%     use-current-rig-position checkbox, custom XYZ fields, rotation fields,
%     and the hover-noise enable + amplitude controls.]}\vspace{3em}}}
  \caption{Hover Settings panel.  When ``Use Current Rig Position'' is
    checked, the butterfly hovers in place at its current world position;
    otherwise the X/Y/Z fields take effect.  Hover Noise produces an organic
    positional wander around that anchor.}
  \label{fig:hover_panel}
\end{figure}

\subsubsection{Step 3b: Path Following Mode}

In Path Following mode, the butterfly flies along a user-drawn NURBS curve.
The user picks the curve from the scene via the \textbf{Select} button in the
Path Settings panel.  Two start modes are available:

\begin{itemize}
  \item \textbf{Start from Path Start} (checked) --- the butterfly teleports
    to the curve's first CV at the start of the simulation and immediately
    begins following the curve.

  \item \textbf{Nearest-point start} (default) --- the butterfly snaps to
    the point on the curve nearest its current rig position.  When the rig
    is more than 0.5\,cm from the curve, an \emph{approach phase} smoothly
    interpolates position (quadratic, with a linear speed ramp), heading,
    and full rotation from the rig pose to the curve snap point before
    cursor-driven path following takes over.
\end{itemize}

The \textbf{Use Path Speed Scale} toggle automatically derives the cursor
advance rate so the butterfly traverses the entire (remaining) curve over
the simulation's duration.  Path Noise can be layered on for organic
left/right/up/down sway around the curve target.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/path_panel.png}
%   \fbox{\parbox{0.50\textwidth}{\centering\vspace{3em}
%     \textit{[PLACEHOLDER: \texttt{img/path\_panel.png} ---
%     the Path Settings sub-panel showing the curve picker, Start From Path
%     Start checkbox, Use Path Speed Scale toggle + scale field, Path Noise
%     toggle + amplitude, and the advanced Deceleration and Angle Blend
%     sub-sections.]}\vspace{3em}}}
  \caption{Path Settings panel.  The Deceleration and Angle Blend
    sub-sections (collapsed by default) provide a smooth landing into a
    user-specified end pose---useful for blending path following into a
    subsequent hover shot.}
  \label{fig:path_panel}
\end{figure}

\subsubsection{Step 3c: Advanced Path Controls --- Deceleration and Angle Blend}

For artists who need a smooth stop at the end of a path (e.g.\ to hand off
into a hover shot), ButterFlight includes two complementary tools:

\begin{itemize}
  \item \textbf{Deceleration.}  Over the last $N\%$ of the curve, the cursor
    advance fades to a small velocity floor using either a linear or
    exponential curve.  The scale-driven \texttt{arcRate} formula is
    automatically rederived to compensate so the cursor still reaches the
    curve end on the final frame even with the slowdown applied.

  \item \textbf{Angle Blend.}  Over the last $N\%$ of the curve, the
    \texttt{BF\_thorax} local rotation is quaternion-slerped from the
    path-tangent-derived pose toward an exact user-specified
    (\texttt{rotateX}, \texttt{rotateY}, \texttt{rotateZ}) target.  On the
    very last frame the thorax key is force-written from the typed Euler
    values, sidestepping any quaternion-to-Euler roundtrip drift in
...
\begin{figure}[H]
  ...
  \includegraphics[width=1.0\textwidth]{img/path_panel.png}
%   \fbox{\parbox{0.55\textwidth}{\centering\vspace{3em}
%     \textit{[PLACEHOLDER: \texttt{img/decel\_angle\_blend.png} ---
%     the Deceleration and Angle Blend sub-panels expanded, showing the
%     enable checkboxes, window-percentage fields, fade-mode option menus,
%     and the three Final Rotate X/Y/Z fields.]}\vspace{3em}}}
  \caption{Advanced path-end controls.  Deceleration (top) fades cursor
    speed; Angle Blend (bottom) interpolates the thorax to an exact
    rotation target on the last frame.}
  \label{fig:decel_angle_blend}
\end{figure}

\subsubsection{Step 3d: Free Flight Mode}

Free Flight runs the full physical model (wing aerodynamics, curl-noise vortex
forces, gravity) without any path or hover constraint.  The butterfly's
velocity emerges from the force model and the inherent trajectory noise of the
curl-noise field.  This mode is useful for ambient background motion or for
seeding a swarm with one leader whose trajectory is not pre-authored.

\subsubsection{Step 4: Cinematic Cameras (optional)}

Two camera tools can be enabled in addition to any simulation mode:

\begin{itemize}
  \item \textbf{Follow Camera.}  Bakes a camera that maintains a fixed offset
    relative to the butterfly across the animation.  When ``Use Current Camera
    Transformation'' is on, the offset, rotation, and FOV are captured from
    the active viewport camera at simulate time, so the artist can frame the
    shot directly in the viewport and ButterFlight inherits exactly that
    framing.

  \item \textbf{Stationary Rotating Camera.}  A camera fixed at the active
    viewport camera's position whose rotation animates each frame to keep
    the butterfly at the same screen-space offset as the framing setup.  The
    optional Auto Zoom keys focal length per frame
    ($f_t = f_0 \cdot d_t / d_0$) so the butterfly stays roughly the same
    apparent size as it moves toward or away from the camera.
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/cameras_panel.png}
%   \fbox{\parbox{0.55\textwidth}{\centering\vspace{3em}
%     \textit{[PLACEHOLDER: \texttt{img/cameras\_panel.png} ---
%     the Follow Camera and Stationary Camera panels side by side, showing
%     the Use Current Camera Transformation toggle, manual offset fields,
%     spring stiffness, FOV, and the Auto Zoom checkbox.]}\vspace{3em}}}
  \caption{Cinematic camera controls.  Both cameras can capture the active
    viewport camera's transform at simulate time so the artist can frame
    shots directly in Maya's viewport and have the bake match exactly.}
  \label{fig:cameras_panel}
\end{figure}

\subsubsection{Step 5: Swarm (optional)}

In any simulation mode, enabling \textbf{Swarm} duplicates the assigned rig
into $N$ agents.  The leader follows the chosen mode (hover, path, or free)
exactly as in the single-butterfly case; the followers run a flocking
behaviour (cohesion, separation, alignment) that loosely tracks the leader's
position while preserving individual variation in flap phase and trajectory
noise.  Spawn spread (cm) and inter-agent repulsion radius (cm) are exposed
as sliders.

\subsubsection{Step 6: Simulate, Scrub, and Render}

Clicking the green \textbf{Simulate} button bakes keyframes onto the rig
joints (and onto any enabled cameras) over the specified frame range.  The
result is standard Maya animation curves that scrub on the timeline and
render without the plug-in loaded.  The blue \textbf{Reset} button restores
all UI fields to defaults; the red \textbf{Delete All Keys} button clears
every animation curve from the rig hierarchy and any ButterFlight cameras
...
This section showcases the typical output an artist can produce with
ButterFlight, drawn from the final demo render.  All four shots below were
generated by ButterFlight without any hand-keyed animation on the rig
joints---only the initial rig pose, the user-drawn NURBS guide curves, and
the simulation parameters were set by the artist.

\subsubsection{UI in Action}

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/ui_in_maya.png}
%   \fbox{\parbox{0.85\textwidth}{\centering\vspace{4em}
%     \textit{[PLACEHOLDER: \texttt{img/ui\_in\_maya.png} ---
%     full Maya screenshot showing the ButterFlight panel docked on the
%     right, the rigged butterfly + NURBS path in the viewport, and the
%     Channel Box / timeline at the bottom with baked animation curves
%     visible.]}\vspace{4em}}}
  \caption{ButterFlight in use inside Maya 2026.  The left side shows the
    viewport with a rigged Monarch butterfly on a NURBS guide curve; the
    right side shows the ButterFlight panel; the timeline shows baked
    keyframes after \textit{Simulate}.}
  \label{fig:ui_in_maya}
\end{figure}

\subsubsection{Shot 1 --- Path Following with Follow Camera}

In the opening shot of the demo, the butterfly flies through a spiky
crystalline terrain along a guided NURBS curve.  A Follow Camera bakes from
the active viewport framing, so the butterfly stays at the same screen
position throughout the flight.  Path Noise is enabled at moderate amplitude
to keep the trajectory from feeling mechanical.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/shot1_terrain.png}
%   \fbox{\parbox{0.85\textwidth}{\centering\vspace{4em}
%     \textit{[PLACEHOLDER: \texttt{img/shot1\_terrain\_followcam.png} ---
%     rendered frame from the demo showing the butterfly mid-path through
%     the spiky-terrain environment, captured by the follow camera.]}
%     \vspace{4em}}}
  \caption{Shot~1 --- path following through a stylised terrain with the
    follow camera locked to the butterfly's framing.}
  \label{fig:shot1}
\end{figure}

\subsubsection{Shot 2 --- Overhead View Showing Path Noise}

The second shot reveals the trajectory's lateral wander from above.  Path
Noise produces an organic, non-repeating left/right swing around the guide
curve, visible as the butterfly weaves slightly off the centerline.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/shot2_overhead.png}
%   \fbox{\parbox{0.85\textwidth}{\centering\vspace{4em}
%     \textit{[PLACEHOLDER: \texttt{img/shot2\_overhead\_pathnoise.png} ---
%     overhead render showing the butterfly's wandered trajectory relative
%     to the underlying NURBS guide curve.]}\vspace{4em}}}
  \caption{Shot~2 --- overhead view making Path Noise's lateral sway
    visible against the underlying guide curve.}
  \label{fig:shot2}
\end{figure}

\subsubsection{Shot 3 --- Deceleration + Angle Blend into a Hover}

In the third shot, the butterfly approaches a white rose at the end of a
guide curve.  Deceleration smoothly fades cursor speed across the last
$\sim$10\% of the curve, while Angle Blend rotates the thorax into a
specified upright pose at exactly the final frame.  The result is a clean
handoff from path following to a stable hover in front of the flower.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/shot3_decel.png}
%   \fbox{\parbox{0.85\textwidth}{\centering\vspace{4em}
%     \textit{[PLACEHOLDER: \texttt{img/shot3\_decel\_hover.png} ---
%     rendered frame at the end of the deceleration + angle blend, showing
%     the butterfly stationary in front of the white rose.]}\vspace{4em}}}
  \caption{Shot~3 --- Deceleration and Angle Blend deliver the butterfly
    into a precise hover pose in front of the rose.}
  \label{fig:shot3}
\end{figure}

\subsubsection{Shot 4 --- Nearest-Point Departure with Stationary Camera}

The closing shot reverses the previous transition: the butterfly leaves the
hover, snaps onto a new NURBS path using the nearest-point start with the
smooth approach phase, and flies off into the distance.  A Stationary
Rotating Camera (with Auto Zoom on) holds its world position while rotating
to track the butterfly and scaling focal length to keep the apparent size
roughly constant as the subject recedes.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/shot4_statcam.png}
%   \fbox{\parbox{0.85\textwidth}{\centering\vspace{4em}
%     \textit{[PLACEHOLDER: \texttt{img/shot4\_statcam\_flyaway.png} ---
%     rendered frame of the butterfly mid-departure, viewed from the
%     stationary camera as it pans and zooms to follow.]}\vspace{4em}}}
  \caption{Shot~4 --- Stationary Rotating Camera with Auto Zoom tracks the
    butterfly as it leaves the rose for the flower field beyond.}
  \label{fig:shot4}
\end{figure}

\subsubsection{Swarm Output}

Although not used in the four-shot demo, swarm simulation is supported in
all three single-butterfly modes.  Figure~\ref{fig:swarm_output} shows a
representative swarm run with $N{=}10$ Monarchs flocking around a leader on a
hover anchor.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{img/swarm.png}
%   \fbox{\parbox{0.85\textwidth}{\centering\vspace{4em}
%     \textit{[PLACEHOLDER: \texttt{img/swarm\_output.png} ---
%     rendered or viewport screenshot showing 10 butterflies flocking around
%     a leader, each with its own flap phase.]}\vspace{4em}}}
  \caption{Swarm output.  Each agent runs the full Module~1--3 pipeline
    independently so flap phase and trajectory noise vary naturally.}
  \label{fig:swarm_output}
\end{figure}
...
% Compare the goals from the Design Doc against what was actually
% built.
% ------------------------------------------------------------

\subsubsection{Features Implemented}

The following features from the Design Doc were fully implemented and ship
with the final build:

\begin{itemize}
  \item \textbf{Full wing kinematics (Module~1).} All five maneuvering angles
    ($\theta_\beta$, $\theta_\gamma$, $\theta_\zeta$, $\theta_\psi$,
    $\theta_\phi$) are evaluated per frame via the cosine maneuvering function
    (Eq.~1) with sigmoid-modulated frequency and amplitude (Eqs.~2--3).
    Per-angle parameters are taken directly from Table~3 of Chen et al.\ 2022.

  \item \textbf{Quasi-steady aerodynamic force model (Module~2).} Per-wing
    lift and drag are computed from the flat-plate approximation (Eqs.~4--6)
    using the empirical polynomial coefficients. Wing-panel normals are derived
    from joint world transforms rather than mesh queries.

  \item \textbf{Curl-noise vortex force (Module~2).} A 3-D Perlin-based
    curl-noise field (Eq.~7) produces turbulent vortex forces at the thorax
    centre of mass, creating the characteristic erratic trajectory noise of
    real butterfly flight.

  \item \textbf{Maneuvering controller (Module~3).} Preferred acceleration
    (Eq.~8), local aerodynamic acceleration (Eqs.~10--11), and sliding-window
    parameter smoothing at cycle boundaries (Eq.~12) are all implemented and
    integrated into the simulation loop.

  \item \textbf{Three simulation modes:} Free Flight, Path Following (with
    cursor-based steering), and Hover.

  \item \textbf{Path following with speed control.} A time-based arc cursor
    guarantees the butterfly traverses the NURBS guide curve over the specified
    duration, with an artist-facing Path Speed Scale multiplier. Both
    ``Start from Path Start'' and nearest-point snap modes are supported.

  \item \textbf{Hover mode.} The butterfly holds a fixed position (either the
    current rig location or user-specified coordinates) with optional
    orientation control (pitch, yaw, roll). Minimum-frequency and
    minimum-amplitude floors keep the wings flapping naturally at zero speed.

  \item \textbf{Swarm / flocking.} Multiple butterflies are spawned with a
    leader--follower flocking model (cohesion, separation, alignment). Each
    agent runs the full Module~1--3 pipeline independently, so flap phase and
    trajectory noise vary naturally across the group.

  \item \textbf{Cinematic cameras.}
    \begin{itemize}
      \item \textit{Follow camera} with viewport-derived offsets
        (``Use Current Camera Transformation'') or manual X/Y/Z offsets.
      \item \textit{Stationary rotating camera} whose position is locked to
        the viewport camera and whose rotation tracks the butterfly with a
        preserved screen-space offset.
      \item \textit{Auto-zoom} for the stationary camera scales focal length
        per frame to keep the butterfly's apparent size roughly constant.
    \end{itemize}

  \item \textbf{Deceleration and angle blend.} Path-following mode can
    smoothly decelerate the cursor across the last portion of the curve and
    blend the thorax orientation into a target pose, enabling clean handoffs
    from path following into hover.

  \item \textbf{Keyframe baking.} All joint rotations and root translations
    are written as standard Maya animation curves, scrubbable on the timeline
    and renderable without the plug-in loaded.

  \item \textbf{Full MEL UI panel.} Ten collapsible panels with
    mode-dependent visibility, parameter validation, and Reset / Delete All
    Keys utilities.
\end{itemize}
...
\begin{itemize}
  \item \textbf{The three-module pipeline from Chen et al.\ held up well.}
    Separating wing kinematics (Module~1), force computation (Module~2), and
    maneuvering control (Module~3) made it possible for two team members to
    develop and test modules in parallel with minimal merge friction.

  \item \textbf{Cursor-based path steering.} Replacing the initial
    velocity-based path-following approach with a time-based arc cursor solved
    the traversal-guarantee problem cleanly. The cursor decouples traversal
    rate from physics velocity limits, and the Path Speed Scale gives the
    artist intuitive control over pacing.

  \item \textbf{Viewport-derived camera offsets.} Capturing the active
    viewport camera's transform at simulate time and computing a quaternion
    offset relative to the look-at direction proved to be a robust and
    intuitive workflow: the artist simply frames the shot, clicks Simulate,
    and the baked camera preserves that framing throughout the animation.

  \item \textbf{Minimum-frequency / minimum-amplitude floors for hover.}
    Rather than fighting the physics pipeline (which produces zero lift at
    zero velocity), bypassing \texttt{controller.step()} entirely in hover
    mode and relying on wing-model floor values produced natural-looking
    hovering animation with minimal implementation effort.

  \item \textbf{MEL-to-C++ flag-based interface.} Keeping all simulation
    logic in C++ and limiting the MEL layer to parameter collection and
    command invocation avoided the performance and debugging difficulties of
    mixed MEL/C++ simulation code.
\end{itemize}

\subsubsection{What Did Not Work}

\begin{itemize}
  \item \textbf{Velocity-based path speed control (first attempt).} The
    original approach computed a desired velocity from curve length divided by
    duration and passed it through the physics blender. The exponential blend
    ($\alpha \approx 0.2\%$ per substep, blend rate~18) converged too slowly,
    and the \texttt{maxSpeed} clamp on both desired and integrated velocity
    made all curves above a certain length appear the same speed. Two
    iteration rounds were needed before the cursor-based design was adopted.

  \item \textbf{Left-wing joint mirroring via negative scale.} The original
    butterfly rig used \texttt{scaleZ = -1} intermediate transforms to mirror
    the left wings from the right. Maya's joint traversal and rotation
    applicator produced incorrect results through these non-standard nodes.
    The fix required deleting and recreating the left-wing joints using
    \texttt{mirrorJoint}.

  \item \textbf{MEL proc name collision with the C++ command.} The MEL
    callback was originally named \texttt{bfSimulate()}, identical to the C++
    plugin command name. Maya resolved the identifier to the MEL proc
    (which took no arguments) instead of the C++ command, causing a
    ``wrong number of arguments'' error. Renaming the MEL proc to
    \texttt{bfSimulateCallback()} fixed the issue, but it required a fresh
    Maya session to clear the cached definition.

  \item \textbf{Skin binding in ``Joint hierarchy'' mode.} Default Bind Skin
    binds the mesh to all joints in the hierarchy, causing one wing's mesh
    to be deformed by both wing joints. Switching to ``Bind to Selected
    Joints'' mode and binding each wing individually resolved this.

  \item \textbf{Focal-length animation on camera shapes.}
    \texttt{clearAnimCurves()} only wipes transform-level plugs; stale
    focal-length keys from prior auto-zoom runs persisted until
    \texttt{clearAnimCurveOnPlug()} was written to handle shape attributes
    explicitly.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Time-varying (sinusoidal) wind.} The Design Doc described an
    optional sinusoidal modulation of the wind vector to simulate gusts. The
    constant wind vector is supported (via the curl-noise vortex force), but
    the time-varying envelope was not implemented. The constant wind already
    produces convincing trajectory perturbation via the curl-noise field, and
    the sinusoidal layer was deemed lower priority than the camera and
    deceleration features that were added instead.

  \item \textbf{Alembic / FBX export button.} The Design Doc included an
    ``Export Cache'' button that would invoke Maya's built-in
    \texttt{AbcExport} or \texttt{FBXExport} after baking. Because the baked
    output is standard Maya keyframe animation, users can export through
    Maya's native File~$\to$~Export dialogs without any special plug-in
    support, so the dedicated button was deprioritized.

  \item \textbf{Vortex-gain and force-parameter UI exposure.} The Design Doc's
    ``Force Parameters'' panel (Section~5) was planned to expose per-axis
    vortex gains (\textit{gain}$_{x,y,z}$), the $\eta$ scale, maximum speed,
    and the sliding-window size $k$ as editable fields. These parameters are
    currently hardcoded to the paper's empirical defaults. Exposing them was
    deprioritized in favour of the camera system and deceleration/angle-blend
    features, which had higher artist impact.
\end{itemize}


% ------------------------------------------------------------
\subsection{Design Changes}
% Were there any changes to the technical approach (i.e. way in
% which you planned to implement the tool)?  What were they?
% What prompted the design change?
% ------------------------------------------------------------

The following design changes were made relative to the original Design Doc
technical approach:

\begin{itemize}
  \item \textbf{Cursor-based path steering replaced velocity-based steering.}
    The Design Doc assumed path following would be driven by computing a
    desired velocity from the path tangent and blending it into the physics
    velocity via the maneuvering controller's exponential blend. After two
    failed iterations (auto-derived speed from curve length, and clamped
    desired velocity), the approach was replaced with a time-based arc cursor
    that advances at a fixed rate per substep. This change was prompted by the
    discovery that the \texttt{maxSpeed} clamp and the slow exponential blend
    made velocity-based speed control fundamentally unreliable for guaranteeing
    curve traversal within a specified duration.

  \item \textbf{Hover mode bypasses the physics pipeline.} The Design Doc
    implied hover would use the same force model with a stationary target.
    In practice, at zero velocity, aerodynamic lift vanishes (Eq.~4 scales
    with $|\mathbf{V}|^2$), gravity pulls the butterfly down, and vortex
    forces push it off station. Rather than adding a PD controller to fight
    these forces, hover mode was implemented by skipping
    \texttt{controller.step()} entirely and running only the wing model with
    minimum-frequency floors. This was simpler, more robust, and visually
    equivalent.

  \item \textbf{Camera system expanded beyond original scope.} The Design Doc
    did not include cinematic cameras. During development, the follow camera
    and stationary rotating camera (with auto-zoom) were added as new features
    to support the demo scene workflow. This was prompted by the realization
    that manually placing and animating cameras to frame a moving butterfly
    was tedious and error-prone, and that viewport-derived offsets could
    automate the process.

  \item \textbf{Deceleration and angle blend added for path-to-hover
    transitions.} These features were not in the original Design Doc but were
    added to solve the jarring discontinuity when a butterfly reaches the end
    of a path curve and transitions to free flight or hover.

  \item \textbf{Swarm architecture changed from centroid-based to
    leader--follower flocking.} The Design Doc proposed a swarm where each
    agent's preferred acceleration targets the swarm centroid. The implemented
    design uses a leader--follower model with Reynolds-style flocking rules
    (cohesion, separation, alignment), which produces more natural group
    behaviour and allows the leader to follow any single-butterfly mode
    (path, hover, or free flight).

  \item \textbf{Camera creation uses MEL \texttt{camera -name} instead of
    C++ node creation.} Creating camera transforms in C++ and renaming them
    was racy (sometimes producing \texttt{camera1} instead of the intended
    name). Using the MEL command atomically creates the transform and shape
    with the correct name.
\end{itemize}


% ------------------------------------------------------------
\subsection{Total Man-Hours Worked}
% Cumulative sum of hours from weekly progress reports, per group
% member.
% ------------------------------------------------------------
...
\begin{tabular}{|l|c|}
\hline
\textbf{Team Member} & \textbf{Total Hours} \\
\hline
Cecilia Chen  & 50 \\
Yiding Tian   & 80 \\
\hline
...
\hline
\end{tabular}
\caption{Cumulative man-hours from weekly progress reports.}
\end{table}


% ------------------------------------------------------------
\subsection{Future Work}
% What additional work is required to make this into a fully
% functional tool usable by an artist?
% ------------------------------------------------------------

The following work would be required to develop ButterFlight into a fully
production-ready tool:

\begin{itemize}
  \item \textbf{Expose tunable physics parameters in the UI.} The per-axis
    vortex gains (\textit{gain}$_{x,y,z}$), curl-noise scale $\eta$, maximum
    speed, and sliding-window size $k$ are currently hardcoded. Exposing them
    as UI sliders would let artists fine-tune the flight character without
    editing source code, enabling different butterfly species or stylized
    flight behaviours.

  \item \textbf{Time-varying wind.} A sinusoidal or noise-driven wind
    envelope would simulate gusts and thermal drafts, adding temporal
    variation to the vortex-force perturbation.

  \item \textbf{Takeoff and landing transitions.} The current simulation
    assumes the butterfly is always in flight. Adding takeoff (leg-push
    momentum) and landing (approach deceleration, leg-contact IK) would
    enable seamless integration into scenes where the butterfly interacts with
    surfaces.

  \item \textbf{Species presets.} The Monarch butterfly parameters from
    Chen et al.\ are hardcoded. A preset system with different wing areas,
    masses, and maneuvering-angle ranges would let the artist switch between
    species (e.g., Swallowtail, Painted Lady) with a single menu selection.

  \item \textbf{Collision avoidance with scene geometry.} Currently the
    butterfly can fly through meshes in the scene. Adding raycasting or
    signed-distance-field queries against the environment would let the
    maneuvering controller steer around obstacles.

  \item \textbf{Performance optimisation for large swarms.} The current
    swarm implementation runs the full Module~1--3 pipeline per agent per
    frame. For swarms larger than $\sim$20 agents, precomputing wing normals,
    batching force evaluations, or using SIMD noise would be necessary to
    maintain interactive performance.

  \item \textbf{Procedural wing deformation.} The current model drives rigid
    joint rotations only. Adding secondary flex and bend via blendshapes or
    a lattice deformer would improve visual fidelity, especially in close-up
    shots.
\end{itemize}
...
% List ALL third party / other software used in the project that
% was not written by the authors.  Brief description of each.
% Also describe any source materials reviewed that were provided
% or made available by the authors.
% ------------------------------------------------------------

\begin{itemize}
  \item \textbf{Autodesk Maya 2026 C++ API (devkit).} The entire plug-in is
    built against the Maya 2026 C++ SDK. The following API classes are used
    extensively:
    \begin{itemize}
      \item \texttt{MPxCommand} --- base class for the \texttt{bfSimulate}
        command; provides flag parsing via \texttt{MArgDatabase} and
        undo/redo hooks.
      \item \texttt{MFnDagNode} / \texttt{MItDag} --- traverse the joint
        hierarchy from the root to locate all \texttt{BF\_*} joints.
      \item \texttt{MFnTransform} --- read and set world-space translation
        and rotation on the root joint each frame.
      \item \texttt{MFnAnimCurve} --- create and write
        \texttt{animCurveTA} (time$\to$angle) and \texttt{animCurveTL}
        (time$\to$linear) nodes for baking joint rotations and root
        position as Maya keyframe curves.
      \item \texttt{MFnNurbsCurve} --- sample the user-specified path curve
        (\texttt{closestPoint}, \texttt{getPointAtParam},
        \texttt{findLengthFromParam}, \texttt{findParamFromLength}) for
        path-following mode.
      \item \texttt{MFnCamera} --- set vertical field of view and read
        focal length for the cinematic camera system.
      \item \texttt{MAnimControl} --- query frame rate and set the playback
        range to the baked frame span.
      \item \texttt{M3dView} --- capture the active viewport camera's
        transform for viewport-derived camera offsets.
      \item \texttt{MVector} / \texttt{MPoint} / \texttt{MMatrix} /
        \texttt{MQuaternion} --- all vector, point, matrix, and quaternion
        arithmetic throughout the simulation.
    \end{itemize}

  \item \textbf{Maya MEL scripting.} The UI panel
    (\texttt{butterFlight\_ui.mel}) is written entirely in MEL using standard
    UI commands (\texttt{window}, \texttt{frameLayout}, \texttt{columnLayout},
    \texttt{optionMenuGrp}, \texttt{floatFieldGrp}, \texttt{button}, etc.)
    and the built-in \texttt{camera} command for atomic camera creation.

  \item \textbf{CMake 3.25+.} Used as the build system to locate the Maya
    devkit and generate the Visual Studio 2022 solution. Not required at
    runtime.

  \item \textbf{Chen et al.\ 2022 reference paper.} ``A Practical Model for
    Realistic Butterfly Flight Simulation'' (ACM Transactions on Graphics)
    provides all equations, parameter tables, and the three-module simulation
    architecture that ButterFlight implements. The paper's Table~3 Monarch
    butterfly parameters are used directly as defaults.
\end{itemize}


% ------------------------------------------------------------
\subsection{AI Tools and Software}
% It is OK to have used AI tools.  Describe how AI was used
% across each phase of the project.
% ------------------------------------------------------------

\subsubsection{Literature Survey}

Claude Code (Anthropic) was used to help locate relevant prior work on
insect flight simulation, aerodynamic modeling, and curl-noise techniques.
It assisted with summarizing candidate papers and identifying which
references were most relevant to the Chen et al.\ 2022 approach.

\subsubsection{Design Doc}

Claude Code (Anthropic) was used to help structure the Design Doc,
including refining the task decomposition in the Work Plan, editing
section descriptions for clarity, and cross-checking that the algorithm
details matched the notation in the reference paper.

\subsubsection{Authoring Tool Software Development}

Two AI tools were used during software development:

\textbf{Claude Code (Anthropic)} was used as the primary AI coding assistant.
It assisted with:
\begin{itemize}
  \item Generating initial implementations of C++ classes
    (\texttt{BFWingModel}, \texttt{BFAerodynamics}, helper functions)
    from the paper's equations.
  \item Writing and iterating on the MEL UI layout, callbacks, and
    mode-switching logic.
  \item Debugging visual artifacts such as incorrect wing orientations
    and joint-rotation anomalies.
  \item Drafting the Final Report \LaTeX{} content.
\end{itemize}

\textbf{Nano Banana (Google Gemini)} was used to generate the butterfly
wing textures applied to the rigged mesh.

% ============================================================
\section{Lessons Learned}
% What was learned during development that the team wishes they
% had known in advance — Maya bugs, helpful documentation, useful
% libraries, API insights, etc., for future CIS6600 students
% developing Maya/Houdini plugins.
% ============================================================

The list below collects the gotchas, undocumented behaviours, and Maya-API
nuances we wish we had known on day~one.  These are written for future
CIS~6600 students who will inevitably hit some of the same walls.

\subsection*{MEL UI and Plug-in Round-Trip}

\begin{itemize}
  \item \textbf{Long flag names must be at least 4 characters.}  We initially
    named the rig flag \texttt{-rig}; Maya treated the 3-character name as a
    short flag and refused to register it (``Invalid flag'').  Renaming to
    \texttt{-rigRoot} fixed the issue.  Lesson: long names $\geq 4$ chars,
    short names $\leq 3$.

  \item \textbf{MEL proc names collide with C++ command names.}  Our MEL
    callback was originally named \texttt{bfSimulate()}, identical to the
    C++ \texttt{MPxCommand}.  When the callback called \texttt{bfSimulate -...}
    via \texttt{eval}, MEL resolved the no-arg MEL proc instead of the C++
    command, producing ``Wrong number of arguments''.  Lesson: always give
    callbacks a different name (we now use \texttt{bfSimulateCallback}).
    A fresh Maya session is required after the rename to clear the cached
    proc table.

  \item \textbf{\texttt{optionMenuGrp -select} is 1-indexed.}  C-style 0-based
    indexing produces ``Invalid menu item'' silently in some Maya versions
    and an off-by-one selection in others.

  \item \textbf{Global variables shadow each other across panels.}  All MEL
    UI control names are stored as globals.  Two panels that both use, e.g.,
    \texttt{\$bf\_pathField} will overwrite each other's control handles
    silently.  Lesson: prefix globals by the panel section consistently
    (\texttt{\$bf\_path*}, \texttt{\$bf\_hover*}, \texttt{\$bf\_cam*}).

  \item \textbf{Re-sourcing the MEL file does not always pick up the new
    procs.}  After a rename or signature change, calling the new name from
    the Script Editor sometimes still hits the cached old definition.  When
    behaviour seems impossibly stale, restart Maya.

  \item \textbf{The plug-in must be reloaded after every rebuild.}  Even if
    Maya's Plug-in Manager says the \texttt{.mll} is loaded, it is the
    \emph{previous} build until \texttt{unloadPlugin} + \texttt{loadPlugin}
    is run.  We wasted multiple sessions debugging ``why isn't my code
    running'' before adding a load-unload step to our test loop.
\end{itemize}

\subsection*{Animation Curves and Keyframe Baking}

\begin{itemize}
  \item \textbf{\texttt{clearAnimCurves(path)} only wipes the transform
    plugs.}  Translation and rotation channels are cleared, but shape-level
    attributes such as \texttt{focalLength} on a camera shape are not
    touched.  When the auto-zoom feature was toggled off between runs, the
    previous run's focal-length curve persisted.  Lesson: walk the plug
    connection of the specific shape attribute and clear it explicitly.
    We added a \texttt{clearAnimCurveOnPlug(path, attrName)} helper that
    follows \texttt{MPlug::connectedTo} to the \texttt{kAnimCurve*} node
    and removes all keys.

  \item \textbf{Pick the right \texttt{MFnAnimCurve} type.}  Rotation keys
    use \texttt{kAnimCurveTA} (time$\to$angle, radians).  Time-to-double
    keys (e.g.\ focal length, scale, custom attributes) need
    \texttt{kAnimCurveTU}.  Translation uses \texttt{kAnimCurveTL}.
    Mismatching the type produces silently-wrong animation that scrubs
    correctly per-frame but evaluates wrong elsewhere.

  \item \textbf{Calling \texttt{addKey} at the same time twice overwrites.}
    This is actually useful: in hover mode and angle blend, we override the
    thorax key after \texttt{writeAllKeys} has already written it, and the
    second \texttt{addKey} replaces the first cleanly.  No need to delete
    first.

  \item \textbf{Quaternion-to-Euler is not unique.}  \texttt{MQuaternion::
    asEulerRotation()} picks a valid Euler representation but it may not
    be the one the user typed (e.g.\ $(180^\circ, -160^\circ, 0^\circ)$
    can come back as $(0^\circ, 20^\circ, 180^\circ)$).  For exact
    final-frame channel-box values, force-write the user's typed Euler
    instead of round-tripping through quaternion.
\end{itemize}

\subsection*{Cameras}

\begin{itemize}
  \item \textbf{C++ \texttt{MFnCamera::create()} + rename is racy.}
    Occasionally the rename failed and the camera ended up named
    \texttt{camera1}, breaking subsequent \texttt{objExists} checks.
    Switching to the MEL \texttt{camera -name "..."} command creates the
    transform and shape atomically with the right name.  Lesson: when
    deterministic naming matters, prefer the MEL command over C++
    \texttt{MFn} create + rename.

  \item \textbf{Focal length is a shape attribute, not a transform attribute.}
    \texttt{MFnTransform} has no idea about it.  Setting it requires
    \texttt{MFnCamera::setVerticalFieldOfView()} or a \texttt{setAttr} on
    the \emph{shape} node.  Same for clearing animation: see the
    \texttt{clearAnimCurveOnPlug} note above.

  \item \textbf{Capture viewport state \emph{before} the simulation's
    seeding time change.}  Our follow camera reads
    \texttt{M3dView::active3dView().getCamera()} for the viewport
    transform.  If we read it after the seeding block jumped Maya's
    current time to a previous frame, we got the camera's transform from
    that other frame instead of the framing the user actually set up.
    Lesson: capture all viewport / scene state up-front, then perform
    time changes.
\end{itemize}

\subsection*{Joints, Skinning, and the Rig}

\begin{itemize}
  \item \textbf{Default Bind Skin uses ``Joint hierarchy''}, not ``Selected
    joints''.  Binding the left wing to the rig hierarchy attached
    \emph{both} wings' meshes to \emph{both} wing joints, producing
    cross-influence the moment one side rotated.  Switch the
    \emph{Bind to} option to ``Selected joints'' and bind each piece
    individually.  Same fix recovered the left antenna from a similarly
    split bind weeks later.

  \item \textbf{Mirroring joints under a negative-scale transform doesn't
    work.}  Maya silently inserts a compensating intermediate transform
    that breaks the joint hierarchy traversal.  Use
    \texttt{mirrorJoint -mirrorYZ -mirrorBehavior -searchReplace "R" "L"}
    to produce a clean mirrored joint chain.

  \item \textbf{Channel-box rotation values are \emph{after} joint orient.}
    When debugging rotation-related bugs, remember that what you read in the
    Channel Box is the \texttt{rotate} attribute, but the world rotation is
    \texttt{parentWorld * jointOrient * rotate}.  Setting
    \texttt{rotate} via \texttt{MFnTransform::setRotation} respects this,
    so it usually behaves as expected, but if a rig's joint orient is
    non-zero you cannot just compare ``my rotation values'' to ``the rig's
    initial pose values'' directly.

  \item \textbf{Scaling the rig after binding breaks the simulation.}
    Skin clusters embed the bind-time joint world matrices.  If the
    artist scales the model after binding, the wing-joint normals computed
    by the aerodynamic solver no longer agree with the visual mesh.
    Lesson: lock final scale before binding, or re-bind after scaling.
\end{itemize}

\subsection*{Frame Rate, Substeps, and Time Units}

\begin{itemize}
  \item \textbf{Decouple physics rate from output FPS.}  Our first build
    used \texttt{simDt = 1/fps} directly, so changing the playback fps
    silently changed the wing flap rate (since \texttt{f*t} also changed).
    Solution: pick a target physics rate (we use $\sim$960~Hz), compute
    \texttt{substeps = ceil(targetHz / fps)}, then \texttt{simDt = simRate /
    (fps * substeps)}.  This keeps the per-substep physics step constant
    regardless of how many frames-per-second the user is rendering.

  \item \textbf{\texttt{simRate} compresses physics time relative to
    playback time.}  In our setup \texttt{simRate $\approx$ 0.182} for
    \texttt{flapPeriod = 1\,sec}, meaning \texttt{m\_state.velocity} is in
    m/sim-sec, not m/playback-sec.  We initially computed an approach-phase
    velocity using \texttt{arcRate * fps * substeps * kCmToM}, which
    produced a value $\sim$5$\times$ smaller than the path-following
    steady-state.  The butterfly visibly stopped at handoff while the
    spring spun velocity back up.  Lesson: when crossing between
    cursor-rate (cm/substep) and physics velocity (m/sim-sec), use
    \texttt{rate * kCmToM / simDt}, never the playback-time multiplier.

  \item \textbf{Maya's frame rate comes from the scene time unit, not from
    a flag.}  We originally exposed an \texttt{-fps} command flag and got
    bug reports about flap speed being wrong on machines whose Maya was set
    to 30~fps.  Reading \texttt{MTime(1.0, kSeconds).as(MTime::uiUnit())}
    fetches the correct value automatically.
\end{itemize}

\subsection*{Asymptotic Curves and Numerical Floors}

\begin{itemize}
  \item \textbf{Decel curves that hit zero never reach the end of the
    integral.}  Both linear $1{-}t$ and normalised exponential
    $\propto e^{-kt}$ go to 0 at $t{=}1$, which makes
    $\int dt/\text{mult}(t)$ diverge.  In a discrete simulation, the
    cursor would advance ever more slowly and the butterfly would never
    actually arrive at the curve end.  Lesson: add a small velocity floor
    (we use 5\% of base rate).  The integral becomes finite, the cursor
    arrives, and visually the curve looks identical to the floor-less
    version because the floor only kicks in for the last $\sim$5\% of the
    decel zone.

  \item \textbf{Quaternion slerp needs short-arc selection.}  Naively
    interpolating $a \to b$ when their dot product is negative produces a
    long way around the sphere.  Flip $b \to -b$ first so dot is positive,
    then slerp.  We also fall back to a normalised \texttt{lerp} when the
    dot product is very close to 1, since $\sin\theta \to 0$ in the
    denominator.

  \item \textbf{Random-phase noise is non-zero at $t{=}0$.}  We added
    organic positional drift to hover mode using
    $\sin(2\pi f t + \phi_1) + 0.5\sin(2\pi f' t + \phi_2)$ with random
    $\phi$.  At $t{=}0$ the sample is generally non-zero, so the first
    frame of the simulation visibly jumped from the rig's anchor position.
    Lesson: precompute the $t{=}0$ sample and subtract it from every
    sample, so the curve passes through zero on frame~1.
\end{itemize}

\subsection*{NURBS Curves}

\begin{itemize}
  \item \textbf{\texttt{MFnNurbsCurve::closestPoint} is well-defined only
    for nearly-planar curves.}  Steeply 3-D paths can produce overshoot or
    multiple local minima.  For path following this was usually fine, but
    we noticed occasional snap-to-wrong-loop behaviour on curves that
    crossed themselves in 3D.

  \item \textbf{Cache total arc length, don't recompute per substep.}
    \texttt{length()} and \texttt{findLengthFromParam} re-evaluate the
    curve internally.  Calling them every substep was a measurable
    slowdown on long paths.  Compute total length once at simulation
    start, only call \texttt{findParamFromLength} per substep.
\end{itemize}

\subsection*{Build and Toolchain}

\begin{itemize}
  \item \textbf{C4819 ``character cannot be represented in current code
    page'' is mostly noise on Windows.}  MSVC emits this for any source
    file containing UTF-8 multi-byte characters when the system code page
    is not UTF-8 (936 / Chinese in our case).  It does not affect the
    compiled binary but it floods the build log and obscures real
    warnings.  Workaround: filter it from the build output, or save the
    affected files as UTF-8 with BOM.

  \item \textbf{Use the Maya devkit headers as the source of truth.}  The
    Autodesk online API docs occasionally lag the headers (e.g.\ for
    Maya~2026's new flags on existing classes).  When a method seems to
    be missing or behaving differently from the docs, check
    \texttt{<maya devkit>/include/maya/MFn*.h} directly.

  \item \textbf{Read the included Maya devkit examples.}  The
    \texttt{<maya devkit>/devkit/plug-ins/} folder ships with worked
    examples for \texttt{MPxCommand}, \texttt{MPxNode}, undoable commands,
    custom locator drawing, etc.  These are far more useful than the
    online API reference for understanding how the pieces fit together
    inside a real plug-in.
\end{itemize}

\subsection*{Maya Viewport Quirks}

\begin{itemize}
  \item \textbf{Viewport 2.0 motion blur lives on
    \texttt{hardwareRenderingGlobals}, not on the camera.}  We hit a
    confusing bug where the viewport became blurry the moment the user
    released Alt after tumbling, but stayed sharp during interaction.
    The cause was scene-level motion blur enabled on
    \texttt{hardwareRenderingGlobals.motionBlurEnable}, which Maya
    suppresses during interactive tumble (so the blurry frame is what you
    see when you let go).  This is unrelated to the plug-in but worth
    flagging because the symptom looked like a viewport bug.
\end{itemize}

\subsection*{General Process Lessons}

\begin{itemize}
  \item \textbf{Add structured logging early.}  Every time we hit a
    ``why isn't my code running'' bug, the answer was either ``a guard
    condition I didn't realise was false'' or ``a code path I didn't
    realise was being taken''.  Sprinkling
    \texttt{MGlobal::displayInfo} calls at every major branch made the
    debugging loop dramatically faster.

  \item \textbf{Implement the simplest end-to-end pipeline first, then
    layer features.}  The first useful build of ButterFlight was a single
    butterfly with constant velocity along a straight line.  Everything
    after that was incremental: forces, sigmoid frequency, sliding-window
    smoother, path following, hover, cameras, swarm.  Trying to wire the
    full force model up before the basic keyframe baking was working
    would have been impossible to debug.

  \item \textbf{Maintain a CLAUDE.md / project-context document.}  Across
    a long project with multiple AI-assisted coding sessions, the single
    most leveraged document was a top-level project description that any
    fresh assistant could read to understand units, joint conventions,
    file layout, and known gotchas.  This kept the AI's suggestions
    consistent with the existing codebase rather than fighting it.
\end{itemize}


% ============================================================
% Bibliography (carried over from the Design Doc)
% ============================================================
